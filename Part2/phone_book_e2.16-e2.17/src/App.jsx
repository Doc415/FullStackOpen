...
const Notification = ({setMessage,isSuccess}) => {
  ...
  console.log(setMessage)
      ...
      return null
        ...
        return(
          <div style={isSuccess?notificationStyleSuccess:notificationStyleError}>
            {setMessage}
          </div>
        )
      }
...
const App = () => {
  ...
  const notificationChange=(message)=>{
    console.log('in notification change')
  ...
  const submitNewName= (event) => {
      ...
      if (persons.some(person=> person.name===newPerson.name)){
        if(window.confirm(`${newName} is already recorded! Would you like to update number?`)){
            ...
            personService.updatePerson(newPerson).then(response=> {
              ...
              setNewPhoneNumber('') 
              setIsSuccess(true)
              notificationChange('Updated successfuly')
            })
            return
        }
        else
      ...
      personService.createPerson(newPerson)
           .then(response=> {
              ...
              setIsSuccess(true)
              notificationChange(`Created successfuly`)
            })
            .catch(error=> {
              setIsSuccess(false)
              notificationChange(`error on creation: ${error}`)}
  ...
  const onDeletePerson= (id) => {
    ...
    personService.deletePerson(id).then(response=>{
                                            ...
                                            setIsSuccess(true)
                                            notificationChange(`User deleted successfuly`) 
                                            console.log(response.status)
                                    })
                                   .catch(error=> {
                                    setIsSuccess(false)
                                    notificationChange(`error on delete: ${error}`)                                    
                                  })       
    }                            
  }
  ...
  return (
    ...
    <div>
      <h2>Phonebook</h2>
      <hr/>
      <Notification setMessage={notification} isSuccess={isSuccess}/>
